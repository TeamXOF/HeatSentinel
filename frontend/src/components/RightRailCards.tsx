...
export const ActiveAlertsCard: React.FC<ActiveAlertsCardProps> = ({
  ...
  return (
    <div
      ...
      className={`bg-white border border-[#F1F5F9] rounded-2xl p-5 shadow-xs flex flex-col gap-4 ${className}`}
    >
      {/* Header */}
        ...
        {alerts.map((alert) => {
          ...
          return (
            ...
            >
              ...
              <div
                className={`w-8 h-8 rounded-full ${iconBg} ${iconColor} flex items-center justify-center shrink-0 mt-0.5`}
              >
              ...
              <div className="flex flex-col min-w-0 flex-1">
                <span className={`text-[13px] font-bold leading-snug tracking-tight ${titleColor}`}>
                  {alert.title}
                </span>
                <span className="text-xs text-[#64748B] mt-0.5 leading-tight">
                  {alert.description}
                </span>
    ...
    >
        ...
        {actions.map((action) => {
          ...
          return (
            <div
              ...
              className="py-3 first:pt-0 last:pb-0 flex items-center justify-between gap-3 cursor-pointer hover:bg-slate-50/80 -mx-2 px-2 rounded-xl transition-colors group"
            >
              {/* Left Number badge + Text */}
              <div className="flex items-start gap-3 min-w-0">
                <div className="w-6 h-6 rounded-full bg-[#FFEDD5] text-[#F97316] text-xs font-bold flex items-center justify-center shrink-0 mt-0.5 tabular-nums shadow-2xs group-hover:scale-105 transition-transform">
                  {action.stepNumber}
                </div>
                <div className="flex flex-col min-w-0">
                  <span className="text-[13px] font-bold text-[#0F172A] group-hover:text-[#0D9488] transition-colors leading-tight truncate">
                    {action.title}
                  </span>
                  <span className="text-xs text-[#64748B] mt-0.5 leading-tight truncate">
                    {action.subtitle}
                  </span>
                </div>
              </div>
              ...
              {/* Right Priority Pill */}
              <span
                className={`px-2.5 py-0.5 rounded-full text-[11px] font-bold uppercase tracking-wider shrink-0 border ${priorityBadgeClass}`}
              >
                {action.priority}
              </span>
            </div>
      ...
      <div className="pt-2">
        <Link
          ...
          className="w-full inline-flex items-center justify-center gap-2 py-2.5 px-4 rounded-full bg-[#0D9488] hover:bg-[#0f766e] text-white text-xs font-bold transition-all shadow-xs hover:shadow-sm"
        >
