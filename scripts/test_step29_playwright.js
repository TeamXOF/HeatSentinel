...
async function runTest() {
  ...
  try {
    ...
    const kpiCards = await page.$$('[id^="kpi-card-"]');
    ...
    const markers = await page.$$('.heat-zone-marker-container');
    console.log(`   - Found ${markers.length} interactive Heat Zone markers on map`);
    ...
    if (markers.length > 0) {
      ...
      // Fallback: Click first priority action or row
      const actionRow = await page.$('[id^="priority-action-row-"]');
      if (actionRow) await actionRow.click();
    }
