...
from app.services.pipeline_service import run_basic_pipeline, load_default_phoenix_target_area
...
async def basic_scan(
    ...
    """
    Executes the end-to-end basic analysis pipeline for Phoenix:
    ...
    cache_key = compute_basic_scan_cache_key(polygon, start_date, start_time, top_n)
    ...
    if not force_refresh:
        with get_db_connection() as conn:
            ...
            if row:
                ...
                logger.info(f"AnalysisRouter: Cache hit for basic-scan ({cache_key})")
    ...
    logger.info(f"AnalysisRouter: Executing live basic-scan pipeline ({cache_key})...")
